...
//components
//context
import { TasksContext, transformHour, } from "../../../../../../../../../contexts/taskContext";
//api functions
...
export const CreateTaskButtons = () => {
    ...
    const componentsOutput = createIsLoading ? (<Spinner>
			{" "}
			<img alt="loading" src={spinner}></img>
		</Spinner>) : (<>
			<ActionsButton onClick={clickHandler} icon={"plusIcon"} text={"Add to calendar"}/>
			<ActionsButton onClick={() => setConfirmDelete({ show: true, dayOfWeek: actualDay })} icon={"minusIcon"} text={"Delete All"}/>
		</>);
    return (<ActionsContainerCommons>
